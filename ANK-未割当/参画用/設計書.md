# AIアプリケーション インフラ設計書 (雛形)

> **本ファイルは未割当案件用テンプレートです** (2026-05-14 追記)
> - 以下の設計は「AI / Kubernetes / Airflow / Databricks 等を扱う案件」向けの参考実装で、未割当案件の確定事項ではありません。
> - 新規案件で利用する際は、案件の実技術スタックに応じて 1.3 技術スタックの各項目を**書き換え or 削除**してください (例: ECS Fargate 中心の案件なら EKS 章を削除、Databricks を使わない案件なら 1.3 から除外)。
> - サンプルがそのまま流用された結果として「Databricks がデータベースとして分類」「StatefulSet をジョブ用途として説明」等の歴史的誤りが残っていた箇所は 2026-05-14 に訂正済みです。

## 1. 概要

### 1.1 目的

本設計書は、AIアプリケーションにおけるインフラ構成、Kubernetes環境、データ処理パイプライン、CI/CDパイプラインの設計方針 (雛形) を定義します。

### 1.2 システム概要

データを収集・分析し、AI技術を活用したプラットフォーム (サンプルシナリオ) を想定しています。

### 1.3 技術スタック (雛形)

新規案件では本欄を実技術スタックで上書きすること。

- **クラウドプラットフォーム**: AWS (案件により GCP / Azure / オンプレ)
- **コンテナオーケストレーション**: Kubernetes (EKS) (案件により ECS / GKE / 不要)
- **IaC**: Terraform / AWS CloudFormation
- **CI/CD**: GitHub Actions (案件により GitLab CI / Jenkins / CodePipeline 等)
- **データ処理 (該当案件のみ)**: Apache Airflow / AWS Step Functions / Kubeflow Pipelines / dbt 等
- **データベース**: SQL (Aurora MySQL / PostgreSQL / RDS) / NoSQL (DynamoDB / MongoDB)。**Databricks はデータベース種別ではなく、Lakehouse / 分析・データ基盤プラットフォーム** (Spark + Delta Lake) のため、該当案件のみ別章で扱うこと
- **プロジェクト管理 / コミュニケーション**: 案件の標準ツール (JIRA / Slack / Notion / Teams 等)
...
- **Deployment**: ステートレスなアプリケーション / AI モデルサービング用 (ローリングアップデート / レプリカ管理)
- **StatefulSet**: 永続 ID / 安定したネットワーク識別子 / 永続ボリュームの順序保証が必要な**常駐ワークロード**用 (例: 自前運用の RDB / メッセージブローカ / Kafka / Zookeeper / Etcd / Redis 等)。**バッチ / ジョブ用途には使わない**
- **Job / CronJob**: 単発・定期実行する**バッチ処理 / データ処理ジョブ**用。`backoffLimit` / `ttlSecondsAfterFinished` を必ず設定。冪等性を担保
- **DaemonSet**: ログ収集 (Fluent Bit) / 監視エージェント (Datadog Agent / node_exporter) / CNI 等、**全ノードに 1 Pod**を配置するワークロード用
...
- **Kubernetes リソース** (マネージド EKS 前提): マネージド EKS の etcd は AWS 管理側で運用されているため利用者は直接 snapshot を取得できない。代替として AWS Backup の **EKS cluster state** バックアップ、および Velero による Namespace / マニフェスト / PV のバックアップ (S3 + バージョニング + Object Lock) を組み合わせる
