...
graph TB
    ...
    subgraph "☁️ AWS VPC: 10.0.0.0/16"
        ...
        subgraph "🔌 VPC Endpoints (Gateway / Interface)"
            S3_EP[📦 S3 Gateway Endpoint]
            DDB_EP[⚡ DynamoDB Gateway Endpoint]
        end

        subgraph "🔐 セキュリティグループ (VPC内のリソースのみ。DynamoDB/S3 は SG を持たない)"
    ...
    subgraph "🌐 AWS マネージドサービス (VPC外 / リージョナル)"
        S3[📦 S3 Data Lake]
        DDB[(⚡ DynamoDB)]
    end
    ...
    EKS_NODES -->|DB接続| RDS
    EKS_NODES -->|API call via VPC Endpoint| DDB_EP
    EKS_NODES -->|API call via VPC Endpoint| S3_EP
    DDB_EP --> DDB
    S3_EP --> S3
    EKS_NODES -->|インターネット egress| NAT
...
| DynamoDB | (SG不適用) | HTTPS/443 | DynamoDB は AWS マネージドサービスで SG / サブネットを持たない。Gateway VPC Endpoint + Endpoint Policy + IAM (リソースポリシー) でアクセスを制御 |
...
graph TB
    subgraph "⎈ Amazon EKS Cluster"
        ...
        subgraph "⚙️ Namespace: data-pipeline"
            AIRFLOW[🔄 Airflow Web/Scheduler<br/>(Deployment) + KubernetesExecutor<br/>(per-task Pods)]
...
- **トレーシング**: AWS Distro for OpenTelemetry (ADOT) を新規実装の標準とする。AWS X-Ray は 2026-02 以降 SDK / Daemon がメンテナンスモードに移行しており、既存資産との互換が必要な場合のみ X-Ray を補助的に併用する
...
- ✅ ADOT (OpenTelemetry) で分散トレーシング (新規実装の標準。AWS X-Ray は既存資産との互換用途のみ)
