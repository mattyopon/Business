...
> **本リポジトリはサンプル / 雛形です。** 医療データを取り扱う案件では、`[担当者名]` `[連絡先]` を**プレースホルダのまま運用すると、HIPAA / 個人情報保護法 (要配慮個人情報) のインシデント対応要件 (連絡網の即時性) に違反する可能性がある**ため、参画時に必ず実名 / 24/365 連絡手段 / 代替経路を確定すること。

| 役割 | 担当 (参画時に確定) | 24/365 連絡手段 | エスカレ先 |
|------|---------------------|----------------|-----------|
| インシデントマネージャー (一次受け) | (参画時に設定) | PagerDuty + Slack `#incident-medical-ai` + 電話 | 副 IM / SRE 責任者 |
| セキュリティチームリード | (参画時に設定) | 電話 + Email | CISO |
| 法務担当 | (参画時に設定) | 電話 (Sev1/PII漏えい疑い時) + Email | 顧問弁護士 |
| 広報担当 | (参画時に設定) | 電話 (公表判断時のみ) + Email | 経営陣 |
| データ保護責任者 (DPO) | (参画時に設定) | 電話 + Email | 監督官庁 (個人情報保護委員会) |
| 医療機器規制対応 (該当時) | (参画時に設定) | Email | PMDA / クラスIIIなら厚労省窓口 |
...
| 2026-05-14 | 1.1 | 連絡網プレースホルダ運用注意の明文化、HIPAA / 個人情報保護法 (要配慮個人情報) の整合補記 | mattyopon |
