...
# Bootstrap .env (Grafana 認証情報など、Compose 起動に必須の変数)
# docker-compose.yml で `${GF_SECURITY_ADMIN_PASSWORD:?...}` を必須化している (弱いデフォルト admin/admin 回避)。
# .env が無いと即失敗するため、.env.example をコピーしてランダムな admin password を自動生成する。
if [ ! -f .env ]; then
