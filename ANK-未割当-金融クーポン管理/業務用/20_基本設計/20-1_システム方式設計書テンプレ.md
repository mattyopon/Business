...
```
   ...
   [PF Egress NW (PF 側集中)]
              ↓
   [Public ALB / API Gateway]  ← 案件側 VPC
              ↓
   [Private Subnet (ECS Fargate)]
              ↓
   [Aurora Cluster (Multi-AZ)]
              ↓
   [PF TGW] → [親システム VPC / 勘定系]
```
...
```
EventBridge (Cron 0:00 JST)
    → ECS Task (顧客取込バッチ)
    → 親システム (バッチ I/F: S3 / SFTP / API)
    → Aurora (クーポン発行対象テーブル)
    → S3 (監査ログ)
```
...
CloudWatch Alarm
    → SNS Topic
    → Slack #ops (Slack Connector)
    → PagerDuty (P1 のみ電話)
    → ServiceNow (チケット自動起票、要件次第)
```
