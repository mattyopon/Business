...
2. 入力バリデーション
3. 監査ログ取得 (受信時)
4. 冪等性チェック (transaction_id でロック取得)
   - 既存 transaction_id → 既存結果を返却
5. クーポン情報取得 (Aurora、SELECT FOR UPDATE)
   - 不存在 → 4xx エラー (理由コード: COUPON_NOT_FOUND)
6. 利用条件判定
   - 有効期限内? → NG なら 4xx
   - 未利用? → 既利用なら 4xx
   - 顧客 ID 一致? → NG なら 4xx
   - 利用金額範囲内? → NG なら 4xx
   - キャンペーン Active? → NG なら 4xx
7. 利用可確定
8. ステータス更新 (Aurora、UPDATE)
9. 監査ログ取得 (判定結果)
10. 勘定系連携 Worker キュー投入 (SQS or 内部処理)
11. レスポンス返却
12. 冪等性ロック解放
```
