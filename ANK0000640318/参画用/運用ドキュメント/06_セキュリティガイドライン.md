...
> **本リポジトリはサンプル / 雛形です。参画時に実連絡先・通報順序・代替経路・最終更新責任者を確定し、`@example.com` / `[電話番号]` のプレースホルダを残したまま運用しないこと。**
> プレースホルダのまま本番運用すると、PCI DSS 12.10.1 (インシデント対応計画の最新性) および 12.10.3 (24時間連絡体制) に違反する可能性がある。

| 役割 | 連絡先 (参画時に確定) | 連絡手段 | 営業時間外の代替 |
|------|---------------------|---------|----------------|
| Security Team | (参画時に設定) | Email + Slack `#security-urgent` | PagerDuty escalation |
| CISO | (参画時に設定) | 電話 (Sev1/Critical時) | 副CISO / 法務責任者 |
| 法務 | (参画時に設定) | Email | 顧問弁護士 |
| 広報 | (参画時に設定) | Email | 広報責任者 携帯 |
| データ保護責任者 (DPO) | (参画時に設定) | Email + 電話 | 個人情報漏えい疑いは即時 |
| 監督官庁 (個人情報保護委員会 等) | 公式窓口 | 法令に従い書面 / フォーム | - |
| AWS / GCP Support | Business / Enterprise Support 経由 | コンソール起票 | Sev1 起票時のみ電話オプション |
...
| 2026-05-14 | 1.1 | 連絡先プレースホルダ運用注意の明文化、PCI DSS 12.10 整合の補記 | mattyopon |
