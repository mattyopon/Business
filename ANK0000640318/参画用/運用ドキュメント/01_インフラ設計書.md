...
> **本ドキュメントは雛形** (2026-05-14 注記)。参画時に実プロダクト名 / 実環境名で確定すること。`[プロダクト名]` 等の placeholder のまま本番利用しないこと。
...
### 1.2 対象システム
- **システム名**: (参画時に確定 / 雛形では `payment-platform` を仮置き)
...
| サブネット | CIDR | リージョン | 用途 |
|-----------|------|-----------|------|
| gke-subnet | 10.0.0.0/20 | asia-northeast1 | GKEノード (Pod / Service secondary range 含む。10.0.0.0-10.0.15.255) |
| db-subnet | 10.0.16.0/24 | asia-northeast1 | Cloud SQL (10.0.16.0-10.0.16.255) |
| proxy-subnet | 10.0.17.0/24 | asia-northeast1 | プロキシ専用 (10.0.17.0-10.0.17.255) |
| public-subnet | 10.0.32.0/24 | asia-northeast1 | 外部 LB / Cloud NAT 等の egress 用 (10.0.32.0-10.0.32.255)。上記 gke/db/proxy と重ならないこと |
...
| バックアップ | 自動（**日次 / 30日間保持**。本書 7.3 バックアップ戦略と整合） |
| ポイントインタイムリカバリ | Enabled（**7日間** の WAL を保持。フルバックアップは別建てで 30 日保持） |
...
| 構成 | regional-asia-northeast1 (本案件初期構成 / 単一リージョン HA) |
| ノード数 | 3 |
| 用途 | 単一リージョン内の強整合トランザクション (例: 決済確定処理)。可用性 SLO は 99.99% を想定 |
| 将来拡張 | グローバル分散が要件化された段階で `nam-eur-asia1` 等のマルチリージョン構成へ移行検討 (コストが約 2-3倍、レイテンシ特性も変化) |

> 注: `regional-*` 構成は単一リージョン内のマルチゾーン冗長であり、グローバル分散ではない。グローバル分散を謳う際は必ず `nam-*` / `nam-eur-*` 系のマルチリージョン構成を採用すること。
...
| 2026-05-14 | 1.1 | Codex レビュー反映 (Cloud SQL backup 保持を 30日に統一 / GKE Autopilot 前提整合 / IAM ロール表記訂正 / プレースホルダ運用注意の明文化) | mattyopon |
